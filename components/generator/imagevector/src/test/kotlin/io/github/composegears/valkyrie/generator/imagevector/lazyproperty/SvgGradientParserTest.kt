...
import io.github.composegears.valkyrie.generator.imagevector.backingproperty.loadIcon
...
class SvgGradientParserTest {
    ...
    fun `svg linear gradient parsing`() {
        val icon = loadIcon("ic_linear_gradient.svg")
    ...
    fun `svg radial gradient parsing`() {
        val icon = loadIcon("ic_radial_gradient.svg")
        val parserOutput = IconParser.toVector(icon)
        val output = ImageVectorGenerator.convert(
            vector = parserOutput.vector,
            kotlinName = parserOutput.kotlinName,
    ...
    fun `svg linear gradient with stroke parsing`() {
        val icon = loadIcon("ic_linear_gradient_with_stroke.svg")
        val parserOutput = IconParser.toVector(icon)
        val output = ImageVectorGenerator.convert(
            vector = parserOutput.vector,
            kotlinName = parserOutput.kotlinName,
