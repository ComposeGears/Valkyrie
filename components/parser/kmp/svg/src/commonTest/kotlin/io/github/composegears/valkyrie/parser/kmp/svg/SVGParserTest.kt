...
internal class SVGParserTest {
    ...
    fun svg_with_invalid_viewBox_throws_IllegalArgumentException() {
        ...
        assertFailsWith<IllegalArgumentException> {
            SVGParser.parse(svg).getOrThrow()
        }
    }
    ...
    fun parse_path_with_stroke_from_SVG() {
        ...
        assertEquals(
            actual = SVGParser.parse(svg).getOrThrow().nodes,
            expected = listOf(
                IrVectorNode.IrPath(
    ...
    fun parse_SVG_file_with_correct_fill_color() {
        ...
        assertEquals(
            actual = SVGParser.parse(content = svg).map { it.nodes },
    ...
    fun parse_file_with_group_rotation() {
        ...
        assertEquals(
            actual = SVGParser.parse(svg).map { it.nodes },
    ...
    fun parse_file_with_group_pivot() {
        ...
        assertEquals(
            actual = SVGParser.parse(svg).map { vector -> vector.groups.map { it.pivotX to it.pivotY } },
            expected = Result.success(listOf(20f to 30f)),
        )
    }
    ...
    fun parse_file_with_group_translate() {
        ...
        assertEquals(
            actual = SVGParser.parse(svg)
                .map { set -> set.groups.map { it.translationX to it.translationY } },
            expected = Result.success(listOf(20f to 30f)),
        )
    }
    ...
    fun parse_invalid_ellipse_from_SVG() {
        ...
        assertFailsWith<IllegalArgumentException> { SVGParser.parse(svg).getOrThrow() }
    }
