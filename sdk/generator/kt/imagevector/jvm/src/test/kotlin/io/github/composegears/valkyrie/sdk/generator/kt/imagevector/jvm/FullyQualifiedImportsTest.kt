...
class FullyQualifiedImportsTest(
    ...
    fun `full qualified import, Color`() {
        ...
        val output = ImageVectorGenerator.convert(
            ...
            config = ImageVectorGeneratorConfig.simple(
                ...
                packageName = DEFAULT_PACKAGE,
                imageVector = ImageVectorConfig(outputFormat = outputFormat),
                fullyQualifiedImports = FullyQualifiedImports(color = true),
            ),
        ).content
    ...
    @Test
    fun `full qualified import, Offset`() {
        val icon = getResourcePath("imagevector/xml/ic_full_qualified.xml").toIOPath()
        val parserOutput = SvgXmlParser.toIrImageVector(parser = ParserType.Jvm, path = icon)
        val output = ImageVectorGenerator.convert(
            vector = parserOutput.irImageVector,
            config = ImageVectorGeneratorConfig.simple(
                ...
                packageName = DEFAULT_PACKAGE,
                imageVector = ImageVectorConfig(outputFormat = outputFormat),
                fullyQualifiedImports = FullyQualifiedImports(offset = true),
            ),
        ).content
