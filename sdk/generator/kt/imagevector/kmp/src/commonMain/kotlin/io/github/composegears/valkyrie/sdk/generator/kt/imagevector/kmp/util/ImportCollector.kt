...
internal fun collectImports(vector: IrImageVector): List<String> {
    val usage = UsageFlags.scan(config, vector)
    val imports = mutableListOf<String>()
    if (
        !config.iconPackTree?.data.isNullOrEmpty() &&
        config.iconPackPackage.isNotBlank() &&
        config.iconPackPackage != config.resolvePackageName()
    ) {
        imports += "import ${config.iconPackPackage}.${config.iconPackTree?.data}"
    }
    ...
    with(usage) {
        if (usesOffset && !config.fullyQualifiedImports.offset) imports += "import androidx.compose.ui.geometry.Offset"
        if (usesBrush && !config.fullyQualifiedImports.brush) imports += "import androidx.compose.ui.graphics.Brush"
        if (usesColor && !config.fullyQualifiedImports.color) imports += "import androidx.compose.ui.graphics.Color"
...
private data class UsageFlags(
    ...
    companion object {
        fun scan(config: ImageVectorGeneratorConfig, vector: IrImageVector): UsageFlags {
            val flags = UsageFlags(usesAddPathNodes = config.imageVector.usePathDataString)
            ...
            fun visit(node: IrVectorNode) {
                when (node) {
                    is IrVectorNode.IrGroup -> {
                        ...
                        when {
                            node.clipPathData.isNotEmpty() && config.imageVector.usePathDataString -> {
                                flags.usesAddPathNodes = true
                            }
                            node.clipPathData.isNotEmpty() -> {
                                flags.usesPathData = true
                            }
                        }
                    ...
                    is IrVectorNode.IrPath -> {
                        when {
                            config.imageVector.usePathDataString -> flags.usesAddPathNodes = true
                        ...
                        when (val fill = node.fill) {
                            ...
                            is IrFill.LinearGradient -> {
                                ...
                                val hasUnnamedColors = fill.colorStops.any {
                                    it.irColor.toName() == null || !config.imageVector.useComposeColors
                                }
                                if (hasUnnamedColors) {
                                    flags.usesColor = true
                                }
