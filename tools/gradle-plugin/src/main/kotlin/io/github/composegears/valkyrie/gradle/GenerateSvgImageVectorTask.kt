...
abstract class GenerateSvgImageVectorTask : DefaultTask() {
    ...
    companion object {
        const val TASK_NAME = "generateImageVector"
        ...
        ): TaskProvider<GenerateSvgImageVectorTask> {
            val taskName = "${TASK_NAME}${sourceSet.name.capitalized()}"
            return target.tasks.register(taskName, GenerateSvgImageVectorTask::class.java) { task ->
                ...
                task.outputDirectory.convention(extension.outputDirectory.orElse(sourceSet.defaultOutputDir()))
