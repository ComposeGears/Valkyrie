...
object BatchIconsValidator {

    fun validate(
        batchIcons: List<BatchIcon>,
        useFlatPackage: Boolean = false,
    ): Map<ValidationError, List<IconName>> {
        return buildMap {
            val brokenIcons = batchIcons.filterIsInstance<BatchIcon.Broken>()
            ...
            val caseInsensitiveDuplicates = batchIcons
                .filterIsInstance<BatchIcon.Valid>()
                .groupBy {
                    // When useFlatPackage is true, all icons go to the same folder regardless of nested pack
                    // So we need to check for duplicates across all nested packs within the same icon pack
