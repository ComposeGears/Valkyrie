 ...
 * Used for inferring icon categories from names and tags when none are provided.
 *
 ...
 */
val CATEGORY_KEYWORDS: List<CategoryKeyword> = listOf(
 ...
 *
 * @param name The icon name (e.g., "arrow-up")
 * @param tags List of tags associated with the icon
 * @return The inferred InferredCategory, or "General" as fallback
 */
fun inferCategoryFromTags(name: String, tags: List<String>): InferredCategory {
    ...
    return bestMatch?.toCategory() ?: InferredCategory(id = "general", name = "General")
}

/**
 * Converts a [CategoryKeyword] to an [InferredCategory].
 */
fun CategoryKeyword.toCategory(): InferredCategory {
    return InferredCategory(id = categoryName.lowercase(), name = categoryName)
}
